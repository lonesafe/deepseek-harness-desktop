...
Account balance queries use the same resolved credentials for both protocols and call `/user/balance` outside the Messages `/anthropic` suffix. Custom gateway prefixes are preserved. Redirects fail without forwarding credentials; monetary fields remain decimal strings.
