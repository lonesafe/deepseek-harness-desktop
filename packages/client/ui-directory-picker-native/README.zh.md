...
两个槽位注册经嵌套的 `ctx.slots.inject()` 调用作为一次事务性效果安装，因为任一声明条目都可能晚些激活或替换其声明。该填充使用共享的 `AsyncPickerFlow` 原语：它在每个 `open` 上升沿只武装一次，因此重渲染永远不会再拉起一个选择器；结算到达持有方最新的处理器，卸载则丢弃结算。线上没有按请求中止的机制，因此被丢弃的宿主侧选择器会一直存活到被答复，而它的答复无处落地。node 半部是一个空 `apply`，让插件留在宿主名单上。
