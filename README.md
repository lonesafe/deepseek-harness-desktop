...
对话页新增**文件**视图，可直接浏览当前对话所属工作区，并预览 Markdown、常见源码与配置文本、图片和 PDF。Markdown 文件可以随时在渲染预览与只读源码之间切换。整个文件管理器保持只读，不会修改项目文件；手机端会自动切换为目录与预览分开的全宽页面。通过官网远程使用时，文件列表与预览也走既有的受控接口中转，无需传输整套 Web 页面。
