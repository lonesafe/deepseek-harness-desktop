...
Call `next()` for the default or return a decision to short-circuit. Pre-policy may deny or ask; `allowed-once` and `allowed-always` proceed, while a non-grant, missing approval channel or service, or agent-less request becomes a denial. Guards may still impose a final denial. Arguments cannot be rewritten because history, audit, UI, and execution must agree.
