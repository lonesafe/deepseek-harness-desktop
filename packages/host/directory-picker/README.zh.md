...
GUI 让操作者通过 OS 选择器或应用内浏览器选择工作区目录。在宿主屏幕上操作时使用原生后端；它的自适应能力也支持远程浏览器列举和创建目录。需要在应用内完成全部选择时使用浏览后端。消费方获得交互类型，并呈现匹配的工作流程。目录选择不会影响 agent loop，浏览流程一次只公开一棵目录树；不支持多根目录。
