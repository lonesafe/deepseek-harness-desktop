...
收尾消息的文件引用使用不指定目标应用的本地化“打开”标签。每次点击都根据已有桌面信息选择原生打开或属主的 Sidebar 预览；缺少元数据时复用当前读取，读取失败仍允许预览。将这一选择放在事件处理函数中，使缓存的正文引用随连接更换选择当前目标，无需第二份元数据订阅。插件释放会阻止等待中的点击操作继续执行。
