...
describe.skipIf(process.env.DSH_EXAMPLE_MODE !== 'lib')('published PDF.js licenses', () => {
  it('keeps every bundled license in the packed client artifact', { retry: 0 }, ({ task }) => {
    expect(existsSync(bundlePath), 'Build the document preview client before running its artifact checks').toBe(true)
