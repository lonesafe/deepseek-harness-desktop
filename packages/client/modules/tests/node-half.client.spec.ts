...
describe('client bundle activation', () => {
  ...
  it('shares script responses until a rebuild or remount replaces the artifact', async () => {
    const packageName = '@fixture/cached-artifact'
    const sibling = '@fixture/cache-sibling'
    const clientPath = writePackage(packageName)
    mkdirSync(dirname(clientPath), { recursive: true })
    writeFileSync(clientPath, 'module.exports = { cachedArtifact: 1 }\n')
    ...
    const [first, concurrent] = await Promise.all([
      routeRequest(route, row().url), routeRequest(route, row().url),
    ])
    ...
    writeFileSync(clientPath, 'module.exports = { cachedArtifact: 2 }\n')
    service.rebuilt(packageName)
    const rebuilt = await routeRequest(route, row().url)
    expect(rebuilt.body.toString('utf8')).toContain('cachedArtifact: 2')
    expect(rebuilt.body).not.toBe(first.body)
    service.rebuilt(packageName)
    expect((await routeRequest(route, row().url)).body).toBe(rebuilt.body)

    entries.pop()
    emitLoaderEntryChange(context, packageName)
    await Promise.resolve()
    writeFileSync(clientPath, 'module.exports = { cachedArtifact: 3 }\n')
    entries.push(packageName)
    emitLoaderEntryChange(context, packageName)
    await Promise.resolve()
    const remounted = await routeRequest(route, row().url)
    expect(remounted.body.toString('utf8')).toContain('cachedArtifact: 3')
    expect(remounted.body).not.toBe(rebuilt.body)
  })
