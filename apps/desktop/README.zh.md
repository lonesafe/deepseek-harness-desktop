...
此包包含共享 dsh Web UI 的两种 Electron 发行版。分支仓库的发布工作流使用 `desktop:dist` 和 `lib/web-main.js`，为 macOS、Windows 和 Linux 提供内置 Electron 运行环境、认证局域网访问、远程控制与官网更新。`package:desktop` 命令使用 `lib/main.js` 和下文说明的官方运行环境；其内置 dsh 资源与更新服务属于该发行版。每个打包流程都显式选择匹配的入口。

分支仓库的账号远程控制将原生文件打开与文件管理器操作保留在本机桌面窗口。远程文件卡片将这些操作标记为不可用，隧道会在转发前拒绝工作区、已声明文件和修改文件的原生打开路由。[远程中转决策](../../.agents/notes/implemented/feature/2026-08-15-account-device-remote-relay.zh.md)定义了其他远程访问限制。
...
macOS Host 构建需要 Xcode Command Line Tools 来编译[内置 Office helper](../../packages/document/office-to-pdf/README.zh.md)。两种 macOS 桌面发行方式均包含其通用可执行文件及私有 kit 适配入口；原生签名与最终运行时清单覆盖该 helper。

Windows 分支暂存流程需要 Visual Studio C++ Build Tools，并将其发布版 CRT DLL 复制到每个内置 Office 可执行文件旁。流程遵循 [Microsoft 的本地部署方法](https://learn.microsoft.com/en-us/cpp/windows/deployment-in-visual-cpp)，因此 Office 预览无需另装 VC++ 运行库。可再分发库或 Office 可执行文件缺失时，暂存流程会报错。
...
分支使用 `--no-open` 启动受管 Web profile，让启动页面保持在 Electron 窗口内。它同样将 pnpm 作为生产依赖随包分发。打包的 `node` 与 `pnpm` 启动器在受管后端 PATH 中优先于系统命令，并使用应用的 Electron 可执行文件，因此包安装与脚本无需系统 Node 或 pnpm。远程访问不能修改插件或 bundle 安装。
...
运行时负载冒烟测试还会验证 POSIX 文件锁的独占获取、竞争拒绝和关闭描述符后的释放。
...
每条打包命令都会构建仓库，打包以 dsh 和私有 Desktop Host 为根的第一方生产依赖闭包，并准备目标专用的 Electron 分发包与 pnpm CLI。`prepare:dsh` 在构建时安装一次生产依赖图，准备物化包供 electron-builder 归档到 `app.asar/dsh`，移除包管理器元数据，并生成包含共享包版本和最终文件哈希的 `desktop-runtime.json`。在 macOS 上，它先签名并验证原生文件，再生成清单；electron-builder 不对已签名的此目录重复进行嵌套签名。资源映射明确包含默认根目录过滤器会忽略的 `dsh/node_modules`；准备完成的运行时清单在原生签名后检查。原生可执行文件、库及完整的 Office 引擎资源解包到 ASAR 旁；Python、独立 Node 和 pnpm 保留在外部 runtime 资源中。签名安装包、公证、已安装应用升级和各目标原生模块的验收需要发布环境。
