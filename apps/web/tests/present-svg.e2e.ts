...
describe('web e2e: requested SVG is explicitly delivered', () => {
  ...
  it('previews a delivered inline file when the Host desktop is unavailable', async () => {
    ...
    const trackNativeRequest = (request: Request): void => {
      if (request.method() === 'POST' && new URL(request.url()).pathname === '/api/present.open') {
        nativeRequests.push(request.url())
      }
    }
    page.on('request', trackNativeRequest)
    try {
      await inline.click()
      const preview = page.locator('[data-document-preview]')
      await expect.poll(() => preview.count()).toBe(1)
      expect((await preview.getAttribute('data-textpreview-url'))?.endsWith(`/${FILE}`)).toBe(true)
      ...
      page.off('request', trackNativeRequest)
    }
  })
})
