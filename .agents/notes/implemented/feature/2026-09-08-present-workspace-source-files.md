...
Closing-message file references use a destination-neutral localized Open label. At each gesture the existing desktop metadata chooses native opening or the owner's Sidebar preview; missing metadata shares the current read and failed reads permit preview. Keeping this choice in the event handler lets cached prose references follow connection replacement without a second metadata subscription. Plugin disposal suppresses pending gestures.
