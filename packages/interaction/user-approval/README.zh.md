...
使用本包可暂停敏感工具操作，等待审批。默认的 `ask` 策略会征询人类或机器应答者，除非当前会话已允许匹配规则；`never` 则在这两条路径之前直接拒绝。应答者缺失或失败时返回 `unavailable`，使操作以拒绝方式关闭。批准可以仅允许一次操作，也可以记住可选的稳定规则，供同一会话中的匹配请求使用。每个请求与结果都会进入会话审计日志；模型会看到工具结果与当前策略，但不会看到权限 UI 或审计事件。
