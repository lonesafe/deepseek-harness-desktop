...
# Persisted-output checks wait for shutdown flushes on shared CI hosts.
SMOKE_SHUTDOWN_TIMEOUT_SECONDS = 30
...
def smoke_sdk_office(executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dsh-sdk-office-") as temporary:
        ...
        with DeepSeekHarness(
            ...
            shutdown_timeout_seconds=SMOKE_SHUTDOWN_TIMEOUT_SECONDS,
        ):
            pass
        ...
        if native.startswith("darwin-") and not result["nativeModuleUrl"].startswith(office.as_uri() + "/"):
            raise AssertionError(f"macOS Office adapter was not loaded from the relocated wheel: {result}")
        pdf = output.read_bytes()
        if len(pdf) < 100 or not pdf.startswith(b"%PDF-"):
            raise AssertionError(f"Office conversion produced an invalid PDF at {output}")
        print(f"smoke-python-runtime: relocated Office provider {expected_backend} DOCX produced {len(pdf)} PDF bytes")
