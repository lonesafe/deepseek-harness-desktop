...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `dsh-desktop`（apps/desktop） | bin 粗分发 + Web 前端 + Electron 生命周期与打包 | 应用不改 Host 组合；dist 与可执行文件定位等 workspace 知识留在 app |
