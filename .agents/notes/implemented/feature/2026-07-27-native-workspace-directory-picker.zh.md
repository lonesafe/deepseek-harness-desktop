...
新增的宿主、运行时、组件和 GUI 测试覆盖原生边界、请求信任校验、取消与故障处理、已有路径复用、同 basename 路径收编和可见名称即时更新。特权 `host.pickDirectory` RPC 仍仅限回环。后续的 adaptive seam 决策让远程页面能够使用独立的目录列举与创建原语，而无需跨越这条原生边界。
