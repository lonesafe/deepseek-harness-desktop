...
The closing prose carries the same vocabulary: an inline-code token resolves by exact path, or by being exactly the basename of exactly one produced path — a basename two paths share stays inert rather than guessing, so a mention can never open the wrong file. A resolved mention keeps its code chip and takes the markdown sheet's link language, with the full path as its title. References use the localized Open label. A declared file opens in the Host's default application when desktop information reports the desktop available at the time of the click; otherwise, including a failed metadata read, it opens in the Sidebar preview. If desktop information has not loaded yet, the click waits for it.
