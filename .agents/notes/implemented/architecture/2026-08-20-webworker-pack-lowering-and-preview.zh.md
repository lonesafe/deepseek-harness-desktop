...
**Tunnel HTTP 请求是回环请求。** 页面与其 dedicated Worker 共享同一份本地部署，因此 tunnel 会把页面提供的 Host 替换为 `127.0.0.1`，并让每个合成的 `IncomingMessage` 携带 `remoteAddress` 为同一回环地址的 socket。产品中间件可在路由分派前读取任一字段；缺少 socket 会让合法插件 bundle 请求变成 WebServer 保护层的 400 响应，而无法抵达 `/plugins`。
