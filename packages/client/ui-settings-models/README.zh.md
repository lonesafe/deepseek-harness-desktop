...
模型设置、产品引导与 DeepSeek 余额插件。同一个 client Cordis 插件会注册 Models 页面、两个有序的首次使用弹窗，以及桌面端与远程 Web 壳共用的余额控件。该控件在宽侧栏底部位于「设置」之后、仅桌面显示的版本／更新控件之前，并且不会进入折叠后的图标栏。余额通过 `llm.balance` 读取 `deepseek-official`，显示各币种精确总额，在提示中给出赠送与充值明细，并在 DeepSeek 设置或凭据变化及重连后刷新；点击余额也可手动刷新。凭据缺失或查询失败时显示 `--`，不会暴露凭据内容。两个引导步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
