...
| Profile | Arguments |
|---|---|
| `web` | `--host`, `--port`, repeatable `--trusted-host`, `--access-token`, `--no-open` |
...
dsh web --dump-config
dsh web --help
dsh web --host 0.0.0.0 --access-token <at-least-24-characters>
```

The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default and, for a local launch, opens that canonical host URL only after the complete Loader tree settles. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use. `--host 0.0.0.0` requires `--access-token` with at least 24 characters; a non-loopback browser signs in on the Chinese login page with username `deepseek` and that token. The server session authenticates later HTTP and WebSocket traffic, while loopback access remains password-free. HTTP Basic is also accepted for non-browser clients. The direct LAN connection is plain HTTP, so use it only on a trusted network. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence; it does not replace authentication.
