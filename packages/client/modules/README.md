...
The node half snapshots each client bundle and available source map before publication. It groups resources into `/plugins/??...&rev=...` combo URLs, with one bootstrap combo for the modules row and one or more application combos for the other rows; each phase is partitioned before a URL exceeds 3 KiB. Every combo map is Indexed Source Map v3 and uses an authored section when available or an identity section for the packaged bundle. Initial per-plugin revisions use process nonces, so startup does not hash every plugin; HMR hashes only an artifact reported as changed. Encoded bundle bytes, serialized map sections, line counts, and one-resource responses are reused across graph changes for each loaded artifact. `rebuilt()` replaces that prepared state when the artifact revision changes; unloading the row releases it. Advertised responses are immutable, and an unknown combination or revision returns 404.
...
- **Snapshot delivery retains artifact bytes** — the Host holds each bundle, optional source map, prepared combo inputs, generated one-resource response, and current startup combo responses in memory; HMR additionally retains one prior startup generation. Memory scales as several copies of the composed client artifacts in exchange for immutable responses and one-generation race tolerance.
