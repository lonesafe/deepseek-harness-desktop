...
async function main(): Promise<void> {
  ...
  try {
    ...
    const officeFiles = ['entry.js', 'NOTICE', ...process.platform === 'darwin' ? ['libreoffice-kit-macos', 'manifest.json'] : []]
    for (const file of officeFiles) {
      const path = join(DSH_OUTPUT_ROOT, 'node_modules', '@deepseek-ai/dsh-office-to-pdf/lib/native', file)
      if (!existsSync(path)) throw new Error(`desktop runtime: missing Office adapter file ${file}`)
    }
