...
describe('browser dependency discovery', () => {
  ...
  it.each(['directory', 'symlink'] as const)('follows shell aliases, CSS and lazy imports through a %s without writing output', async (access) => {
    const root = fixture()
    ...
    const inputRoot = access === 'symlink' ? join(root, 'checkout') : root
    if (access === 'symlink') symlinkSync(root, inputRoot, 'junction')
    expect(await browserBundledExternals(inputRoot)).toEqual(new Set(['shell-lib', 'lazy-lib', 'asset-lib']))
