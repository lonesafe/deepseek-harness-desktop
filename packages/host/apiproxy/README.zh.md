...
`workspace.listFiles` 与 `workspace.readFile` 为对话文件管理器公开单个已注册 Workspace 的有界只读投影。请求只携带可移植的 Workspace 相对路径。Host 每次调用都会解析已注册的规范根目录，拒绝绝对路径、父级／点／空路径段和反斜杠变体；符号链接只有在真实目标仍位于该根目录内时才会被跟随。单次目录响应最多 1,000 项。单次文件响应最多 8 MiB，并把 UTF-8 Markdown／文本、常见位图、PDF 与其他二进制内容分类，供预览或下载；更大的内容只报告元数据，不返回字节。文件系统竞态与不可读路径会转成稳定的 `workspace-file-*` 业务错误，成功结果不会暴露 Host 绝对路径。
...
`llm.balance` 是只读的账户余额路由。请求点名一条已注册提供方路由，响应只包含账户可用状态以及精确十进制的总额、赠送额与充值额行；提供方凭据绝不进入响应。提供方、传输、凭据与响应校验失败统一折叠为 `balance-unavailable`，details 只携带提供方路由。桌面远程策略允许转发该方法，因为它只暴露金额汇总，不暴露配置或 secret 值。
...
- **待处理交互状态位于宿主侧**：审批与问题注册表经 POST `/api/respond` 加 `RpcReceipt` 跨客户端重连保留可应答请求。审批帧会声明是否可使用会话级已记住授权；`respond` 只为这些条目接受 `allowed-always`，规则身份始终只保留在 Host。
