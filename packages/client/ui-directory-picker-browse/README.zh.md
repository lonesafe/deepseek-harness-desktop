...
应用内目录浏览界面：它通过 ui-workspace 的两个 directory-flow 洞（`conversation.hero.workspace.directoryFlow` 与 `sidebar.workspaces.directoryFlow`）装入「选择工作区目录」对话框，经 `ctx.workspaces` 驱动 `host.listDirectory` 与 `host.createDirectory`。普通 browse 模式始终渲染该对话框。启用 `nativeOnLoopback` 时，同一界面对回环桌面页面无渲染地驱动 `host.pickDirectory`，对远程页面则保留目录浏览对话框，使同一 adaptive 桌面 Host 能服务两种操作者位置，而不把原生选择器暴露给远程页面。它的 node 对侧是 [`dsh-host-directory-picker-browse`](../../host/directory-picker-browse/README.zh.md)。
