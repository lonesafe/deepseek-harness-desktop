...
Chat restores semantic anchors across history prepend and renderer remounts. Approaching the history start automatically loads an older event page and preserves the visible anchor; the button remains an explicit fallback. While the reader is pinned to the floor, `ResizeObserver` follows the new floor and selects the latest loaded Turn without reading row geometry. A pending scroll delivery blocks that follow only when its position differs from the component's observed-top ledger; deferred follows retry after sampling, and a partial native-anchor move toward a growing owned floor completes instead of releasing ownership. A guarded pre-paint pass covers renderer layout that finishes after the resize callback. Once the reader moves away, flow-height changes preserve the top position and the reading-line geometry selects the active Turn. Turn-rail previews paint above sticky Markdown code-block banners, while the rail frame remains inside the transcript band above the composer ([scroll attribution](../../../.agents/notes/implemented/bug-fix/2026-08-06-reader-scroll-attribution-observed-top-ledger.md), [loaded-Turn navigation](../../../.agents/notes/implemented/feature/2026-08-25-loaded-turn-chat-navigation.md)).
...
- **The transcript reflects the loaded Session window** — older transcript nodes become available only after Session Controller loads the preceding event page, whether triggered near the history start, by the fallback button, or by Turn navigation. Turn navigation is wider than the window: the rail merges the loaded Turns with the host `turnOutline` projection, so every started Turn gets a fixed-pitch mark (10px apart; a ladder taller than the frame scrolls inside it with gradient fades), and activating an unloaded mark pages history through the Turn's `turn/start` seq before landing on its row. Without the projection (assemblies not mounting `dsh-session-turn-outline`) the rail falls back to loaded Turns only.
