...
Escalating bash calls resolve `ctx.approval` before execution. Both grant outcomes apply the requested mode only to the current call; `allowed-always` additionally remembers the bash-and-target-mode approval for matching escalations in this session. Rejection, cancellation, unavailability, or missing approval context executes nothing and returns a distinct error. On a real denial, the model may retry the same command once in the same turn with the narrowest sufficient mode and justification; the approval prompt itself is the consent step. Escalation is never speculative, and a disabled or rejected approval is final. The [sandbox Agent Note](../../../.agents/notes/implemented/feature/2026-07-06-sandbox.md) owns the rationale.
