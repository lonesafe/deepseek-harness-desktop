...
- `cordis.yml` 决定交互形态；`apps/cli` 挂 [`-auto` 选择器](../feature/2026-07-29-directory-picker-adaptive-default.md)，它在启动时判定宿主处境。有人值守桌面端挂 adaptive 原生后端和配置为仅在回环使用原生选择器的 browse 表层；headless 宿主挂固定 browse 组合。直接组合某个后端行仍会固定交互。
