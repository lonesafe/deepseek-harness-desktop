...
describe('ui-settings-general apply', () => {
  ...
  it('registers the remote-access row only for an injected desktop renderer', async () => {
    vi.stubGlobal('location', {
      search: '?dsh_desktop_version=1.0.0-beta.5&dsh_desktop_platform=darwin&dsh_desktop_arch=arm64'
        + '&dsh_update_origin=https%3A%2F%2Fdsh.roubsite.com',
    })
    try {
      const b = await bench()
      declare(b.slots)
      ...
      expect(b.slots.entries('settings.general.item')[0]!.component).toBe(DesktopRemoteAccessRow)
    } finally {
      vi.unstubAllGlobals()
    }
  })
