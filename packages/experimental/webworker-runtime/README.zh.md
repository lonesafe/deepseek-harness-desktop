...
[已构建 bundle 导入检查](tests/compile/transform-corpus-check.ts)在裸 Node 中检查库产物，不使用工作区源码别名。Dockkit 例外仅接受 Node 针对固定构建样式表报告的未知扩展名错误；其他失败以及意外成功的豁免导入仍然报错。参见 [CI 观察决策](../../../.agents/notes/implemented/testing/2026-09-08-ci-completion-observations.zh.md)。
