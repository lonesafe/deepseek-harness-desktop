...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const overlayPatches: PatchOptions[] = [
    ...
    // Browser snapshots allow Pong delivery under shared-host contention.
    // See .agents/notes/implemented/testing/2026-09-19-browser-fixture-heartbeat.md.
    { id: 'typert-gateway', config: { websocketHeartbeatIntervalMs: 10_000 } },
...
/** Give pre-V3 seed streams positive relative timing before their final wall-clock rebase. */
 ...
 * V3 and newer preserve embedded stream timing; older fixtures receive positive relative timestamps.
...
export function parseSeedFixture(fixtureText: string): {
  ...
  const events = parseSessionLog(current).map((event) => {
    if (typeof sourceHeader.version === 'number' && sourceHeader.version >= 3) return event
