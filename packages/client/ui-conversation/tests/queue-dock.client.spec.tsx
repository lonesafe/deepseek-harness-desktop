...
describe('QueueDock', () => {
  it('clears the save tooltip while the edit is pending and after controls return', async () => {
    const snap = snapshotWith([row('i-edit', 'before')])
    const source = liveSession(snap)
    const pending = Promise.withResolvers<undefined>()
    const view = render(
      <QueueDock {...kitFor(snap, { updateQueue: () => pending.promise })} useSession={source.useSession} />,
    )
    fireEvent.click(view.getByLabelText('编辑排队消息'))
    const save = view.getByLabelText('保存排队消息')
    fireEvent.focus(save)
    expect(view.getByRole('tooltip').textContent).toBe('保存排队消息')
