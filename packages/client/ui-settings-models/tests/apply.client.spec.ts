...
describe('ui-settings-models apply', () => {
  ...
  it('registers the models nav entry for declarations before or after apply', async () => {
    ...
    const balanceInjected = (
      utility.inject as unknown as
      () => import('../src/client/BalanceIndicator.tsx').BalanceIndicatorInjected
    )()
    expect(balanceInjected.hooks.balance.getSnapshot().status).toBe('error')
    expect(balanceInjected.refresh).not.toThrow()
