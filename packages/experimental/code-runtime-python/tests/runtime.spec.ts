...
describe('PythonCodeRuntime — hostile peer', () => {
  ...
  it('checks and encodes a wide completion value in O(depth), not O(width)', async () => {
    ...
    //
    // The CPU and wall ceilings are 120s, not the 20s the memory assertion alone needs: the O(depth)
    // cursor pulls 6M elements one at a time through Python-level frames, which costs
    // ~11s on an idle machine and more under V8 instrumentation or host indexing.
    // This budget bounds the run without letting a loaded runner's scheduling latency
    // read as a `timeout` — what this test asserts is the O(depth) memory use, not speed.
    const { runtime } = await setup({
