...
describe('scope tree', () => {
  it('keeps lifecycle snapshots stable while publishing every transient chunk', async ({ bench }) => {
    const b = bench()
    await feedList(b, [{ id: 's1' }])
    ...
    expect(b.mock.log.streams(FOLLOW).filter(stream => stream.state === 'open')).toHaveLength(0)
