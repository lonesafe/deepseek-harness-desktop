...
在 `packages/host/` 落一个三包能力 seam——`directory-picker`（Service Definition）、`directory-picker-native`、`directory-picker-browse`（后端）——唯一约定方法 `capability()` 返回**可辨识联合**。固定原生后端返回 `{ kind: 'native', pick(signal) }`，固定浏览后端返回 `{ kind: 'browse', list(path?), createDirectory(path, name) }`；同时服务自身窗口与已认证远程页面的有人值守桌面端返回 `{ kind: 'adaptive', pick(signal), list(path?), createDirectory(path, name) }`。网关（`dsh-host-apiproxy`）注入 `directoryPicker`，提供该 kind 可用的 RPC，并对不可用交互返回 `directory-picker-unavailable`。联合之所以可辨识，是因为固定后端差异在**交互形态**；adaptive 成员则有意携带两种形态，让同一个桌面进程能按浏览器页面选择可达的交互。

**client 侧靠 slot 组合，而非按 wire 广播分支。** ui-workspace 的两个触发表层各自声明一个 `single` 目录流洞（`conversation.hero.workspace.directoryFlow`／`sidebar.workspaces.directoryFlow`；之所以是两个 key，是因为一个洞只有一个声明它的 slot entry——owner 约定相同、占用者相同）。固定浏览部署注册应用内的选择工作区目录对话框；固定原生部署可以注册驱动 `host.pickDirectory` 的无渲染流程。自动桌面组合则注册一个 adaptive 浏览表层包：当前页面为回环时运行原生选择器，否则展示由 `host.listDirectory` 与 `host.createDirectory` 支撑的应用内浏览器。洞的 owner 会话（`open`/`busy`/`onPicked`/`onCancel`/`onError`）承载整个交换：ui-workspace 保留触发（菜单入口仅在洞被占用时渲染）与接纳（`createWorkspace({path})`、可重试的错误对话框、重新选择），占用者持有从 `open` 到所选路径之间的一切。一行组合仍会把交互作为整体切换，同时挂两个流程包会在 client 加载期失败（`single` 洞）。早先的 `host.describe.directoryPicker` 广播与客户端 capability kind 分支继续保持删除；唯一分支来自 connection 层已经可信的页面 authority。洞注册表（`ctx.slots.entries`）仍是每次打开菜单的占用读取。
...
- `cordis.yml` 决定交互形态；`apps/cli` 挂 [`-auto` 选择器](../feature/2026-07-29-directory-picker-adaptive-default.zh.md)，它在启动时判定宿主处境。有人值守桌面端挂 adaptive 原生后端和配置为仅在回环使用原生选择器的 browse 表层；headless 宿主挂固定 browse 组合。直接组合某个后端行仍会固定交互。
