...
- **全语料统一要求。** 范围内的每篇文档从创建起就必须有完整配对；政策没有逐文件推进状态或日期分界。仓库根目录首页是唯一针对发行版的例外，由[中文默认仓库首页决策](2026-08-15-chinese-default-repository-readme.md)记录。README 发现会覆盖除此以外所有位于 vendor 源码、依赖目录与被忽略的构建产物目录之外且文件名不区分大小写匹配 README 的文件，包括今后新增的顶层目录。发布到文档站的配对使用 `pairedPages()`，由根 locale 投影 `.zh.md`，由 `/en/` 投影 `.md`；仅创建对侧文件并不会发布它。
