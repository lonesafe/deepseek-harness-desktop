...
The [native runtime probe](../../../../apps/desktop/scripts/native-runtime.mjs) runs through Electron with `ELECTRON_RUN_AS_NODE=1` before packaging and in `afterPack`. It resolves the deployed application → dsh → Session persistence dependency chain and rejects package entry paths outside the application. POSIX verification acquires a lock on a probe-owned temporary file; importing the lazy flock wrapper alone does not load the addon. Windows verification calls Koffi's `GetCurrentProcessId`, and Linux additionally requires an application-owned executable Landlock launcher. This checks both the staged pnpm graph and electron-builder's copied graph. [Probe tests](../../../../apps/desktop/tests/native-runtime.spec.ts) cover native-operation failure, external dependency resolution, and a missing Linux launcher.
...
**Rebuild fs-ext for Electron.** The Session writer uses the shared Node-API addon rather than a NAN dependency with an Electron-specific ABI. Rebuilding an unused package cannot qualify the deployed writer; exercising its actual native operation does.
