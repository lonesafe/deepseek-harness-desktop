...
**Tunnel HTTP requests are loopback requests.** The page and its dedicated Worker share one local deployment, so the tunnel replaces the page-supplied Host with `127.0.0.1` and gives every synthesized `IncomingMessage` a socket whose `remoteAddress` is the same loopback address. Product middleware may use either field before route dispatch; omitting the socket turns a valid plugin bundle request into the WebServer's guarded 400 response instead of reaching `/plugins`.
