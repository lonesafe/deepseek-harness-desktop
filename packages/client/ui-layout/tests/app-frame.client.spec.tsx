...
describe('AppFrame', () => {
  ...
  it.each([1920, 390])('switches only the keyed main outlet when the active panel changes at %ipx', (width) => {
    frameWidth = width
    ...
    for (const panelId of ['panel-a' as MainPanelId, 'panel-b' as MainPanelId, null]) {
      ...
      expect(tracks(frame)).toEqual([width === 390 ? 0 : 280, 0])
      expect(frame.hasAttribute('data-sidebar-overlay')).toBe(width === 390)
