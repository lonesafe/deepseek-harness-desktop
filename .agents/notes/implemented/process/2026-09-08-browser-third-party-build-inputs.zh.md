...
分类检查保留源码执行方式。许可证解析也不能依赖已有 `lib/` 或写入构建输出；其测试覆盖真实解析、纯类型擦除、资源引用与缺失依赖。依赖发现将仓库根目录转换为真实路径，使 Vite 相对根目录的 HTML 名称与解析后的模块 ID 使用相同的文件系统路径，包括通过目录符号链接访问的情况。独立消费静态包或公开类型的开发者需要自己提供相应构建依赖；本决策不增加独立浏览器库的支持承诺。
