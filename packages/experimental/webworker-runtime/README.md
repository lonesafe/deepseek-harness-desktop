...
The [built-bundle import sweep](tests/compile/transform-corpus-check.ts) checks library artifacts in plain Node without workspace source aliases. Its Dockkit exception accepts only Node’s unknown-extension error for the pinned built stylesheets; other failures and unexpectedly successful exempt imports remain errors. See the [CI observation decision](../../../.agents/notes/implemented/testing/2026-09-08-ci-completion-observations.md).
