...
Entry-level mounting remains load-bearing because `dsh-client-modules` discovers the client surface from the Loader entry exactly as it discovers a configuration row. Per-page selection uses the already-derived `ctx.remote.$host.isLoopback` fact and does not advertise the host capability on the wire. Both the standalone native surface and the adaptive surface drive their asynchronous string pick through the zero-Cordis `AsyncPickerFlow` primitive in `dsh-client-ui-primitives`; feature packages retain their own wire calls and composition. The desktop tunnel continues to block `directoryPicker/pick` for remote pages, whose selection uses only the forwarded `directoryPicker/list` and `directoryPicker/createDirectory` methods.
...
- **A feature plugin importing the native flow and falling back after `directory-picker-unavailable`.** Rejected: client bundle purity forbids cross-feature runtime imports, and per-call probing pays a doomed RPC on every open of a browse host. The adaptive browse surface branches on page authority before starting an operation and shares only a neutral lifecycle primitive from the static UI layer.
...
- **Copy the native request lifecycle into the adaptive surface.** Rejected: the two copies had identical open-edge, latest-callback, StrictMode, and unmount-settlement behavior and failed the duplication gate. Moving that Cordis-free lifecycle to `dsh-client-ui-primitives` preserves feature-package isolation without suppressing the clone.
