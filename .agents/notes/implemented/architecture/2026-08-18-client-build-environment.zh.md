...
**把默认构建送入桌面安装包。** 默认 profile 有意把开发产物标为本地构建，并省略只属于官方构建的注册项。把这些产物装进安装器，即使桌面壳的打包检查通过，也会让桌面产品携带开发环境的身份与行为。

## Consequences

Vite 静态壳和共享 tsdown 动态 bundle 对同一 `DSH_CLIENT_*` 构建进程变量产生相同字符串值。未设置的静态点访问得到 `undefined`，非 `DSH_CLIENT_*` 值不会通过该机制进入浏览器产物，业务代码也无法枚举构建进程环境。每次完整构建都携带可公开展示的短源码 revision。CI 构建门禁和桌面发布准备都选择官方 profile，而不把其中的公开值暴露给源码测试或无关 workflow 步骤。桌面开发构建仍可使用默认 profile，安装器则只 staging 官方 profile 的 client 产物。npm 打包与 built Web 测试会校验记录中的环境及当前产物摘要，因此默认构建后请求官方打包、局部重建或修改输出都会在消费产物前失败。
