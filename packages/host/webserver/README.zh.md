...
- **不终止 TLS**：直接局域网连接中的登录凭据与应用流量使用明文 HTTP；只应在可信网络中使用，或在服务器前方部署 TLS 反向代理。
- **认证使用一份进程级凭据**：不提供按设备区分的账号、撤销记录或限速；更换配置 token 需要重启服务器。
