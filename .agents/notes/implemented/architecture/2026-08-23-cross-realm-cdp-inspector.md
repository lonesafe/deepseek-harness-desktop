...
The Client Console observer preserves the original page call and asynchronously emits one event per enabled DevTools session. Client activation returns an exact `client-console/enabled` frame, and `Runtime.enable` does not complete until the Worker receives that acknowledgement from every connected Client. A Console call made after the enable response therefore cannot overtake the browser-side subscription. A Client that connects later keeps execution-context creation ahead of its DOM insertion while activation completes asynchronously. Each session serializes arguments into its own `console` object group, so disconnect, Runtime disable, or `Runtime.discardConsoleEntries` can release one connection without invalidating another. Context and Fiber arguments use the same semantic reference and DOM reverse mapping as evaluation results.
...
- Host and Client Console events use the same projector; Client activation is acknowledged before `Runtime.enable` completes, arguments remain isolated by DevTools connection, and Cordis arguments resolve to Elements nodes.
