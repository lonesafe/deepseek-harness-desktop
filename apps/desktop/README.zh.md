...
macOS arm64 命令要求 Apple Silicon。macOS x64 命令可以在 Intel macOS 或带 Rosetta 的 Apple Silicon 上运行。Windows x64 命令要求 Windows x64。Linux 安装器使用分支仓库的 `desktop:dist:linux` 发行流程；独立种子打包器没有 Linux 目标。
