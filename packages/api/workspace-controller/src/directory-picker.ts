...
const createDirectoryRequestSchema = z.object({
  ...
  request => request.name.trim() !== '' && request.name !== '.' && request.name !== '..'
    && !/[/\\]/.test(request.name),
  { message: 'directoryPicker.createDirectory requires a single non-blank path segment name' },
)
...
export class DirectoryPickerController extends TypertRemoteService {
  ...
  async createDirectory(path: string, name: string): Promise<string> {
    ...
    if (!request.success) {
      throw new RemoteError(
        'gateway/bad-request',
        'invalid payload for directoryPicker.createDirectory',
  ...
  private requireCapability<Kind extends keyof DirectoryPickerCapabilities>(
    ...
    if (capability.kind !== kind && capability.kind !== 'adaptive') {
      throw new RemoteError(
