...
describe('plugin registration', () => {
  it('registers the tail entry and fiber disposal removes it', async () => {
    ...
    const delivered = tailOwner({ produced: [], presented: [{ path: 'report.docx', seq: 2, index: 0 }] }, 3,
      (path) => { opened.push(path) })
    const reference = service?.forClosing(delivered, SessionId('child-session'))?.resolve('report.docx')
    expect(reference?.label).toBe('Open report.docx')
    ...
    fetcher.mockResolvedValueOnce(Response.json({ name: 'remote', available: false, fileManager: 'finder' }))
    await face.reloadPresentedHost()
    fetcher.mockClear()
    reference?.open()
