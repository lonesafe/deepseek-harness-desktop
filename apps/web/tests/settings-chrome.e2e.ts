...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens a browser asking for no shipped language in English', async () => {
    ...
    try {
      ...
      await dialog.waitFor({ timeout: 10_000 })
      await dialog.getByRole('button', { name: 'English' }).waitFor({ timeout: 10_000 })
      const agentPreset = dialog.getByRole('button', { name: 'Standard mode' })
      await agentPreset.waitFor({ timeout: 10_000 })
      await expect.poll(() => agentPreset.isEnabled(), { timeout: 10_000 }).toBe(true)
