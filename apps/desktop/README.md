...
The macOS arm64 command requires Apple Silicon. The macOS x64 command runs on Intel macOS or Apple Silicon with Rosetta. The Windows x64 command requires Windows x64. Linux installers use the fork’s `desktop:dist:linux` distribution; the isolated seed packager has no Linux target.
