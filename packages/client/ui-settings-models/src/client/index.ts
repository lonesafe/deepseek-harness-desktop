  ...
  ctx.effect(() => {
    ...
    const disposers = [
      ctx.remote.$on('settings/document-updated', (namespace) => {
        refreshModels()
        if (namespace === 'llm-deepseek') void balanceController.load()
      }),
      ctx.remote.$on('credentials/updated', (reference) => {
        refreshModels()
        if (reference === 'DEEPSEEK_API_KEY') void balanceController.load()
      }),
