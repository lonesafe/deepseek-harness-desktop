...
Reader input is no longer identified by device. ChatView keeps an observed-top ledger (`observedTopRef`): the last `scrollTop` either delivered on the main thread or written by the component, recorded synchronously at every programmatic write site — bottom follow, open restore, prepend anchoring, resize follow, and scroll delivery itself. When a scroll event arrives, a position that deviates from `min(ledger, floor)` by more than half a pixel is reader input; a position on the ledger (a delayed programmatic delivery) or exactly on the shrunken floor (a browser clamp after content shrank) preserves the current ownership state. Ownership then changes only through reader input under the existing threshold rule: within `FOLLOW_THRESHOLD` of the floor re-pins, beyond it releases follow and shows Back to bottom. Attribution listens to `scroll` alone, so wheel, touch, scrollbar, keyboard, and any future input source are covered by one rule. A bubbling transcript key handler separately routes unmodified Home, End, PageUp, and PageDown from non-editable nested controls to the resolved conversation scrollport because Chromium does not consistently choose that outer scroller; it leaves the ledger unchanged so the resulting scroll event still establishes reader ownership through the same device-independent rule.

Prepend anchoring resolves ownership from the resulting geometry. After restoring the anchored row, ChatView compares the final scroll position with the new floor, updates `atBottom`, and persists either the normalized reader position or the pinned sentinel. The position before the prepend cannot retain bottom ownership after inserted history moves the floor away from it.
...
Unit specs in `packages/client/ui-conversation/tests/chat-view.client.spec.tsx` pin the ledger contract directly: a `readerScroll` helper delivers a position the component never wrote, programmatic deliveries land on the ledger, the stream-finalization shrink clamp keeps following, and paging keys from a nested non-editable control move the resolved scrollport while modified keys and editable targets remain untouched. Two scenarios in `apps/web/tests/chat-scroll-contract.e2e.ts` extend the [browser e2e lane](../testing/2026-07-24-web-gui-browser-e2e-lane.md): keyboard paging over a settled transcript and a touch-style momentum fling against paced streaming, both red under the wheel-only implementation and green under the ledger.

The prepend unit scenario also verifies that an anchored position which is no longer near the expanded floor releases follow, stores its normalized position, and exposes Back to bottom. `apps/web/tests/stats-paged-history.e2e.ts` exercises the same transition through automatic history pagination.
...
Loading older history can change bottom ownership even though the reader did not emit a scroll event. That transition is deliberate: the post-prepend position is the authoritative state, so follow controls and durable restoration agree with what the reader sees.
