...
macOS uses a bundled universal helper with a [Quartz completion-wakeup repair](../../../.agents/notes/implemented/bug-fix/2026-09-19-macos-office-import-wakeup.md). A checksum-pinned private kit entry preserves the original engine, font worker, validators, and child environment. `pnpm run build:office-native` prepares source-checkout assets; release packages include them and require no compiler. Missing or incompatible assets report `unavailable`. The [native build](native/build.mjs) verifies source pins, both CPU architectures, and artifact hashes before signing; application signing owns the final executable bytes. Ordinary Windows and Linux installs use the unmodified kit entry. ASAR installs retain the macOS compatibility helper or the original helper on other platforms; only native filesystem paths map to unpacked resources, while JavaScript and font-worker imports retain logical archive URLs.
