...
The Python bootstrap resolves the Office kit and the macOS provider’s asset locator from the adjacent Office directory so native helpers and URL Workers use real filesystem paths. The provider code remains in the executable. The kit and provider select their installed engines; the Python bootstrap adds no runtime download or compilation.
...
The installed-wheel smoke creates a clean virtual environment outside the checkout, proves the installed distribution and executable identities, then exercises default and customized SDK profiles, external plugins, MCP, native tools, direct JSON-RPC, committed snapshots, and the real provider on trusted runs. Its Office scenario relocates the complete target payload and converts DOCX through the shipped Office provider with the required platform engine: the target’s declared native engine, or WASM when no native engine is declared. See the [Python contributor workflow](../development.md) and [installed-wheel testing decision](../../.agents/notes/implemented/testing/2026-08-23-installed-python-wheel-black-box-ci.md).
