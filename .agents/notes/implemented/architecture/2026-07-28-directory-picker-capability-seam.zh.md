...
在 `packages/host/` 落一个三包能力 seam——`directory-picker`（Service Definition）、`directory-picker-native`、`directory-picker-browse`（后端）——唯一约定方法 `capability()` 返回**可辨识联合**。固定 native provider 返回 `{ kind: 'native', pick(signal) }`，固定 browser provider 返回 `{ kind: 'browse', list(path?), createDirectory(path, name) }`，而同时服务自身窗口与已认证远程页面的有人值守桌面返回 `{ kind: 'adaptive', pick(signal), list(path?), createDirectory(path, name) }`。`dsh-api-workspace-controller` 中的 `DirectoryPickerController` 注入 `directoryPicker`，提供该 kind 可用的生成 Remote 方法，不可用交互以 `directory-picker-unavailable` 应答。固定 provider 之间的差异在交互形态；adaptive 成员刻意同时携带两种形态，让一个桌面进程能够逐个浏览器页面选择可达交互。

**client 侧靠 slot 组合，而非按线上广播分支。** ui-workspace 的两个触发表层各自声明一个 `single` 目录流洞（`conversation.hero.workspace.directoryFlow`／`sidebar.workspaces.directoryFlow`；两个 key 共享同一 owner 约定与占用者）。固定 browser 部署注册应用内的「选择工作区目录」对话框；固定 native 部署可以注册无渲染的 `directoryPicker/pick` 流程。自动的有人值守桌面组合只注册一套 adaptive browse 界面：当前页面为回环时运行原生选择器，否则呈现由 `directoryPicker/list` 与 `directoryPicker/createDirectory` 支撑的应用内浏览器。洞的 owner 会话（`open`/`busy`/`onPicked`/`onCancel`/`onError`）承载整个交换：ui-workspace 保留触发与接纳，占用者持有从 `open` 到所选路径之间的一切。一行组合仍把交互作为整体切换，同时挂两套流程包会在 client 加载期失败（`single` 洞）。早先的能力广播继续保持删除；唯一分支读取 connection 层提供的页面 authority。洞注册表（`ctx.slots.entries`）仍是每次打开菜单的占用读取。
...
- `cordis.yml` 决定交互形态；`apps/cli` 挂 [`-auto` 选择器](../feature/2026-07-29-directory-picker-adaptive-default.zh.md)，它在启动时判定宿主处境。有人值守的桌面挂载 adaptive native 后端，并把 browse 界面配置为只在回环页面使用 native；无头宿主挂载固定 browse 组合。直接组合某个后端行仍可固定交互。
