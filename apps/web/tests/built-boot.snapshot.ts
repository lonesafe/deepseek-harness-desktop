...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (Reflect.get(clientBuildEnvironment, 'DSH_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="26 0 156 24"]')).not.toBeNull()
    expect(screen.queryByText('DSH Local Build')).toBeNull()
  } else {
    // Desktop/local distributions intentionally keep the generic shell brand;
    // only an explicit official profile installs the upstream wordmark seats.
