...
describe('plugin registration and config', () => {
  ...
  it.each([
    [new TypeError('offline'), undefined, 'TRANSPORT'],
    [new DOMException('cancelled', 'AbortError'), 'aborted', 'ABORTED'],
  ] as const)('maps balance transport failure %s to %s', async (failure, state, code) => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(failure)
    const abort = new AbortController()
    if (state === 'aborted') abort.abort()
    try {
      await expect(adapterOf().accountBalance('deepseek-official', state === undefined ? undefined : abort.signal))
        .rejects.toMatchObject({ code, cause: failure })
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  it.each([
    [
      new Response(JSON.stringify({ error: { message: 'credential denied' } }), { status: 401 }),
      'credential denied',
      'AUTH',
    ],
    [new Response('not JSON', { status: 502 }), 'DeepSeek balance API error (HTTP 502)', 'SERVER'],
  ] as const)('maps a balance HTTP failure through provider details', async (response, message, code) => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(response)
    try {
      await expect(adapterOf().accountBalance('deepseek-official'))
        .rejects.toMatchObject({ message, code, failure: { status: response.status } })
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  it.each([
    ...
    [Response.json(null), 'non-object response'],
    [Response.json({ is_available: 'yes', balance_infos: [] }), 'invalid response fields'],
    [Response.json({ is_available: true, balance_infos: [null] }), 'non-object row'],
  ] as const)('rejects a balance response with %s', async (response, _label) => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(response)
    try {
      await expect(adapterOf().accountBalance('deepseek-official'))
