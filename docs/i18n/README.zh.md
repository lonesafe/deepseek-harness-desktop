...
**范围**：根目录 CONTRIBUTING 与 BRAND_GUIDELINES 文档、除仓库根目录首页外的全部非 vendor README，以及 `.agents/notes/**`、`docs/**` 与 `python/**` 下的全部活跃文档。匹配 README 时只看文件名且不区分大小写，因此今后新增的目录无需再修改 manifest。依赖目录、被忽略的构建产物目录以及冻结的 `.agents/notes/archived/` 目录树只在发现阶段排除，不属于持续演进的翻译源文档。
...
- 仓库根目录的 [README.md](../../README.md) 与 [README.en.md](../../README.en.md)：GitHub 将不带语言后缀的文件渲染为默认首页，因此本桌面发行版在默认路径提供中文，并在 `.en.md` 同级文件中提供英文。
