...
describe('first-party Session format catalog', () => {
  ...
  it('statically owns the complete adjacent chain through the current writer', () => {
    const header = {
      type: 'session',
      version: 0,
      id: 'catalog',
  ...
  it.each(['current', 'transformed'] as const)('rejects native current obsolete required tags with %s validation and retains ignorable tags', (validation) => {
    const header = deepFreeze({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'native-ptc', createdAt: 1, isSeeded: false, delegationDepth: 0 })
