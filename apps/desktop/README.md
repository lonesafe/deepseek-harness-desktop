...
This package contains two Electron distributions of the shared dsh Web UI. The fork release workflow uses `desktop:dist` and `lib/web-main.js` for macOS, Windows, and Linux, with its bundled Electron runtime, authenticated LAN access, remote control, and portal updates. The `package:desktop` commands use `lib/main.js` and the official runtime described below; their bundled dsh resources and update service belong to that distribution. Each packager selects its matching entry explicitly.

The fork's account-based remote control keeps native file opening and file-manager actions in the local desktop window. Remote file cards report those actions as unavailable, and the tunnel rejects workspace, declared-file, and changed-file native-open routes before forwarding them. The [remote relay decision](../../.agents/notes/implemented/feature/2026-08-15-account-device-remote-relay.md) defines the other remote access limits.
...
macOS Host builds require Xcode Command Line Tools for the [bundled Office helper](../../packages/document/office-to-pdf/README.md). Both macOS Desktop distributions include its universal executable and private kit adapter; native signing and the final runtime inventory cover the helper.

Windows fork staging requires Visual Studio C++ Build Tools and copies its release CRT DLLs beside each bundled Office executable. It follows [Microsoft's local deployment procedure](https://learn.microsoft.com/en-us/cpp/windows/deployment-in-visual-cpp), so Office previews require no separate VC++ runtime installation. Missing redistributable libraries or Office executables reject staging.
...
The fork starts its managed Web profile with `--no-open`, keeping startup in the Electron window. It also ships pnpm as a production dependency. Its packaged `node` and `pnpm` launchers precede system commands on the managed backend PATH and use the application’s Electron executable, so package installation and scripts need no system Node or pnpm. Remote access cannot change plugin or bundle installations.
...
The macOS arm64 command requires Apple Silicon. The macOS x64 command runs on Intel macOS or Apple Silicon with Rosetta. The Windows x64 command requires Windows x64. Linux installers use the fork’s `desktop:dist:linux` distribution; the isolated runtime packager has no Linux target.
...
The runtime payload smoke also verifies exclusive POSIX locking, rejected contention, and release after closing the descriptor.
