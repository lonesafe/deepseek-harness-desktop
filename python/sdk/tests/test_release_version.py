...
MACOS_OFFICE_ASSETS = ("package.json", "lib/native/entry.js", "lib/native/manifest.json", "lib/native/NOTICE", "lib/native/libreoffice-kit-macos")


def _write_macos_office_adapter(archive: zipfile.ZipFile, root: str, invalid: str | None = None) -> None:
    for name in MACOS_OFFICE_ASSETS:
        if name == invalid:
            continue
        asset = zipfile.ZipInfo(f"{root}/@deepseek-ai/dsh-office-to-pdf/{name}")
        asset.external_attr = (0o644 if invalid == "mode" else 0o755) << 16
...
def test_office_wheel_requires_only_target_engine(
    ...
    with zipfile.ZipFile(wheel, "w") as archive:
        if platform_tag.startswith("macosx_"):
            _write_macos_office_adapter(archive, root)
...
@pytest.mark.parametrize("invalid", [*MACOS_OFFICE_ASSETS, "mode"])
def test_office_wheel_requires_complete_executable_macos_adapter(tmp_path: Path, invalid: str) -> None:
    root = "runtime-office/node_modules"
    wheel = tmp_path / "office.zip"
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr(f"{root}/@deepseek-ai/libreoffice-kit/package.json", "{}")
        engine = {"kind": "wasm", "loader": "loader.cjs", "wasm": "engine.wasm", "data": "engine.data", "metadata": "fonts.json"}
        base = f"{root}/@deepseek-ai/libreoffice-kit-wasm"
        archive.writestr(f"{base}/prebuilds.json", json.dumps({"engine": engine}))
        for field in ("loader", "wasm", "data", "metadata"):
            archive.writestr(f"{base}/{engine[field]}", b"payload")
        _write_macos_office_adapter(archive, root, invalid)
    with zipfile.ZipFile(wheel) as archive:
        with pytest.raises(RuntimeError, match="macOS Office"):
            build_python_release.verify_office_payload(archive, root, "macosx_14_0_arm64")
