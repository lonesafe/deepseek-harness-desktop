...
def verify_office_payload(archive: zipfile.ZipFile, office_modules: str, platform_tag: str) -> None:
    ...
    if target.startswith("macos-"):
        provider = f"{engines}/dsh-office-to-pdf"
        for name in ("package.json", "lib/native/entry.js", "lib/native/manifest.json", "lib/native/NOTICE", "lib/native/libreoffice-kit-macos"):
            asset = f"{provider}/{name}"
            if asset not in names:
                raise RuntimeError(f"macOS Office asset is missing: {asset}")
        helper = f"{provider}/lib/native/libreoffice-kit-macos"
        if (archive.getinfo(helper).external_attr >> 16) & stat.S_IXUSR == 0:
            raise RuntimeError(f"macOS Office helper lost its executable bit: {helper}")
