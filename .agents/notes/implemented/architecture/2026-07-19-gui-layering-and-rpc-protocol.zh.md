...
> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的直连 Web 实现由 HTTP 上行加 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.zh.md)组成，远程官网可选择[复用 WebSocket RPC 载体](2026-08-16-remote-websocket-rpc-carrier.zh.md)，浏览器对象层见 [Web 客户端架构笔记](2026-07-19-gui-web-client-architecture.zh.md)。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 通过受控的随机回环 origin，把同一套 Web 组合打包进 Electron；该应用边界归[桌面启动器笔记](2026-08-15-electron-desktop-launcher.zh.md)所有。
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，vite 应用）+ `dsh-desktop`（apps/desktop） | bin 粗分发 + 每个应用一个拼装模块（web.ts / headless.ts）+ Electron 生命周期与打包；vite 应用是 `dsh-client-web` 壳表面之上的薄 main | 各应用使用动态 import，因此不会互相加载；dist 与可执行文件定位等 workspace 知识留在 app |
...
1. **选 fetch 伪造方式**：浏览器或桌面 renderer 的同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（见下文「子类表」）。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

现有应用保持这一区分：Web 命令挂载 Host、载体与浏览器组合；桌面启动器把该命令作为子进程管理，并渲染其回环 origin；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| `WebApiClient` | dsh-client-connection | 直连 `globalThis.fetch` 或官网选择的复用 `/api/rpc` 上行 + 每逻辑流一条同源 WebSocket 下行 | 浏览器客户端；物理边界见 [WebSocket 下行](2026-08-04-websocket-downlink-carrier.zh.md)与[远程 RPC 载体](2026-08-16-remote-websocket-rpc-carrier.zh.md)决策 |
