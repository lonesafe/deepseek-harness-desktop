...
The conversation includes a **Files** view for browsing the current conversation's Workspace and previewing Markdown, common source and configuration text, images, and PDFs. Markdown files can switch between rendered preview and read-only source at any time. The file manager is entirely read-only and never changes project files; on phones, it switches to separate full-width directory and preview pages. The same controlled API relay carries listings and previews during portal remote use, without relaying the complete Web shell.
