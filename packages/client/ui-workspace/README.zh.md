...
共享 Workspace 浏览器、选择器与文件预览插件。`WorkspaceBrowser` 填充侧边栏的 `sidebar.workspaces` slot，`WorkspacePicker` 填充页面局部 Session Intent 主视觉区的 `conversation.hero.workspace` slot，`WorkspaceFilesView` 则在 `conversation.view` 中为当前 Session 所属 Workspace 注册**文件**视图。浏览器与选择器使用同一套 Workspace 菜单和添加流程。
...
对话中的**文件**视图通过 `WorkspaceRuntime.listFiles()` 逐层浏览目录，并通过 `WorkspaceRuntime.readFile()` 预览所选普通文件。Markdown 使用共享渲染器显示，UTF-8 文本及常见源码／配置格式直接显示，受支持的位图与 PDF 可预览，受限内容还可下载。隐藏条目默认不显示，可从工具栏开启。浏览器只暴露可移植的 Workspace 相对路径；Host 侧的包含关系检查会拒绝父目录穿越，以及任何逃出已注册规范根目录的符号链接。整个界面强制只读：单次目录响应最多 1,000 项，超过 8 MiB 的文件不返回内容。手机宽度下，目录和预览会切换为两个全宽页面，并提供明确的返回操作；视图同时为动态输入框和安全区预留与对话一致的空间。
...
三个目标 slot 都由其他插件声明，因此 `apply` 使用 `slots.inject()` 在各自的声明生命周期内完成注册，并在目标 slot 的声明恢复后重新注册。
...
无。浏览器、选择器与文件预览都属于客户端界面；预览内容不会加入模型请求。
...
- **文件管理器只读且有明确上限**：它不支持编辑、重命名、上传或删除文件；单次列表在 1,000 项处停止，预览在 8 MiB 处停止，PDF 能否内嵌显示取决于浏览器能力。
