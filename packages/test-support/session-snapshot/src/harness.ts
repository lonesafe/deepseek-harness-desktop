 ...
 * failures identify the Session and deadline and retain the underlying cause.
  ...
  try {
    await vi.waitFor(async () => {
      ...
      const matched = log?.content.split('\n').some((line) => {
        ...
        return record.type === 'agent/inbox/spliced' && record.data?.inserted?.some(inserted =>
          inserted.content?.some(block => block.type === 'text'
    ...
    // The deadline can precede the first harvest, before the callback names the missing insertion.
    throw new Error(message, { cause })
  }
}
