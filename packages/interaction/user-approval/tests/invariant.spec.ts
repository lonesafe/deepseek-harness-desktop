...
describe('approval invariants', () => {
  it('accepts paired audit events and closed policy values', async () => {
    ...
    session.append('approval/asked', { id, toolName: 'bash', alwaysAllowKey: 'sandbox:bash:workspace-write' })
    session.append('approval/decided', { id, outcome: 'allowed-always' })
  ...
  it('rejects malformed and unpaired audit events', async () => {
    ...
    expect(() => session.append('approval/asked', { id, toolName: 'bash', alwaysAllowKey: '' }))
      .toThrow(/alwaysAllowKey must be non-empty/)
    session.append('approval/asked', { id, toolName: 'bash' })
