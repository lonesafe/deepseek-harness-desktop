...
describe('plugin registration and config', () => {
  ...
  it.each([
    ...
    ['messages', 'https://gateway.example/proxy/anthropic', 'https://gateway.example/proxy/user/balance'],
    ['messages', 'https://gateway.example/proxy', 'https://gateway.example/proxy/user/balance'],
  ] as const)('reads exact account balances for %s at %s', async (protocol, baseURL, balanceURL) => {
    ...
    try {
      const adapter = adapterOf({ protocol, baseURL, apiKey: 'balance-key' })
