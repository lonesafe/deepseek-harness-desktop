...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 通过受控的随机回环 origin，把同一套 Web 组合打包进 Electron；该应用边界归[桌面启动器笔记](2026-08-15-electron-desktop-launcher.md)所有。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 面向普通浏览器与 Electron renderer 的浏览器形态承载；零 workspace 依赖（注册表经结构注入到达） |
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，Vite 应用）+ `dsh-desktop`（apps/desktop） | bin 粗分发 + 每个 CLI 应用一个拼装模块（web.ts / headless.ts）+ Electron 生命周期与打包；Vite 应用与 Electron renderer 都消费 `dsh-client-web` 壳表面 | CLI 应用使用动态 import，因此不会互相加载；应用不改 Host 组合；dist 与可执行文件定位等 workspace 知识留在 app |
