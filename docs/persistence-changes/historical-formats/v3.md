...
Inspect the complete V3 persistence declarations captured at `dsh-v0.1.5-rc.2`. System prompts use message events, surface events require placement metadata, and PTC dispatches use their dedicated event names. This reference preserves the selected official release checkpoint, including its original field types and optionality.
...
The source is the published tag `dsh-v0.1.5-rc.2`, whose writer constant is 3 and whose source date is 2026-09-10T13:50:19.000Z. It is the latest V3 tag in the [prerelease archive](../releases/README.md). The [tag record](../releases/dsh-v0.1.5-rc.2.md) records no persistence-root changes from its predecessor. This complete inventory includes unchanged roots reconstructed from the preceding archived releases.

The declarations come from `packages/core/session/src/types.ts`, `packages/session/session-persistence-jsonl/src/format.ts`, and every first-party `SessionEventMap` merge and its reachable types. Source paths below identify historical declarations; they do not refer to current file contents.

<a id="characteristics"></a>
## Format characteristics

These facts describe the selected checkpoint. The generated tables below preserve its exact declarations.

- `SessionHeader.version` resolves to the literal 3. Both logical and physical headers require `isSeeded`; the physical header declares `version` as a number.
- `system/message` stores system prompt messages. `request/header.data.header` has no `system` field.
- All four surface event types require `surfaceOp`; replacement endpoints are `startSeq` / `endSeq`. `assistant/message` has no `sourceEventSeqs` field.
- `tool/ptc-dispatch` and `tool/ptc-dispatch-start` replace the V2 code-dispatch event names. Assistant messages and attempts retain embedded `stream` data.

The [V3 codec](../../../packages/session/session-format-v2-to-v3/src/codec.ts) owns physical framing and source-reference decoding. The [V2-to-V3 specification](../../../packages/session/session-format-v2-to-v3/README.md#v2-to-v3-specification) owns conversion of system messages, local references, PTC names, and canonical envelopes; its [native V3 admission rules](../../../packages/session/session-format-v2-to-v3/README.md#native-v3-admission) define validation beyond these declarations.
...
roots:
  ...
  "event:assistant/message": a58ca1bed502d425a8b65cac02f59717498cd296de9513c6f81b83b791c20e9b
  ...
  "event:system/message": 0ae7ccdc1cc5d9d43772ebd5c7357ff509011c3a6ba188d9c274d33df2542c42
  ...
  "event:tool/ptc-dispatch": da807583a19f4ebe2bc9115c9686016e1521e405bdfaced296ae652b91e8e5f6
  "event:tool/ptc-dispatch-start": ec38b5949af8eacaf00df002f4acbe344f934f8a061e9cdc65a52a48e5f6dd93
  "event:tool/result": cc3e270ab982cbafff602b753647b6f056decb7fe4fd523afd2f157a65c3d622
  ...
  "event:user/message": 2e49ede7fb3a592cce8919ebe982f6ca1c1f67abe4def359ffc028c85e6d1ad8
...
The [complete machine inventory](v3.schema.json) contains 59 roots and 462 reachable types. Digests include all referenced fields; source names and paths describe the selected historical tree.
...
| Property | Presence | Type |
|---|---|---|
| `dt` | required | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
...
| `shadowedSeqs` | required | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
...
| `ignorable` | optional | `true` |
| `seq` | required | `number` |
| `sourceEventSeqs` | optional | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
| `surfaceOp` | required | [`packages/core/session/src/types.ts#SurfaceOp`](#persistence-type-packagescoresessionsrctypestssurfaceop) |
| `time` | required | `number` |
...
| `messageSeqs` | required | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
...
- [`SessionEventEnvelope[36]`](#persistence-type-sessioneventenvelope36)
...
<a id="persistence-type-sessioneventenvelope36"></a>

#### `SessionEventEnvelope[36]`
...
<a id="persistence-type-sessioneventenvelope36sourceeventseqs"></a>

#### `SessionEventEnvelope[36].sourceEventSeqs`
...
This inventory reconstructs the selected tag from the validated [prerelease archive](../releases/README.md), retaining every root and exactly its reachable types. The repository's canonical graph and digest helpers validate the result; no current-source types replace historical declarations. The archive's extraction and normalization limits still apply.

The selected official checkpoint does not represent every later V3 payload revision. The fork already wrote `allowed-always` in `approval/decided` under V3; that value is absent from this snapshot. The [V4 acknowledgement](../2026-09-19-desktop-remembered-approval-v4.md) records its first formal reconciliation and preservation requirements.

Declared types do not establish codec admission or migration safety. Opaque values, external plugin events, physical body encoding, and runtime relationships remain outside this inventory's coverage.

```sh
pnpm run verify-persistence-formats
```

The check validates every format through the current writer, schema completeness, bilingual declarations, and generated tables using only this checkout. It does not replay historical applications. See the [format index](README.md) for maintenance rules and the [current catalog](../../persistence-catalog.md) for today's declarations.
