...
A fourth surface, its own settings page (`settings.section` id `agent-presets`, ordered after Models — choosing a model is routine, composing an agent is the deployment-shaping act behind it): the roster as cards, a copy dialog as the only way a preset is created, and a read-only viewer over the shipped compositions. This authoring section is registered only for a loopback page; remote Web keeps the ordinary preset selector and running-session label but does not show controls backed by loopback-only authoring RPCs.
