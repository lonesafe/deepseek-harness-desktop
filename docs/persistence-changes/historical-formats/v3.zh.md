---
description: "完整的 Session V3 持久化类型、源码证据与物理编码参考。"
...
查看 `dsh-v0.1.5-rc.2` 捕获的完整 V3 持久化声明。系统提示词使用消息事件，surface 事件要求位置元数据，PTC 分发使用专用事件名。本参考保留所选官方发布检查点的声明，包括原始字段类型和可选性。
...
来源是已发布标签 `dsh-v0.1.5-rc.2`，其写入版本常量为 3，源码日期为 2026-09-10T13:50:19.000Z。它是[预发布归档](../releases/README.zh.md)中的最新 V3 标签。[标签记录](../releases/dsh-v0.1.5-rc.2.zh.md)记载了相较前一版本没有持久化根类型变更。本完整目录包含从前序归档发布重建的未变更根类型。

声明来自 `packages/core/session/src/types.ts`、`packages/session/session-persistence-jsonl/src/format.ts`，以及所有第一方 `SessionEventMap` 合并声明及其可达类型。下方源码路径标识历史声明，不指向当前文件内容。

<a id="characteristics"></a>
## 格式特征

这些事实描述所选检查点。下方生成表保留其精确声明。

- `SessionHeader.version` 解析为字面量 3。逻辑与物理 header 均要求 `isSeeded`；物理 header 将 `version` 声明为数字。
- `system/message` 存储系统提示词消息。`request/header.data.header` 没有 `system` 字段。
- 全部四种 surface 事件类型都要求 `surfaceOp`；替换端点为 `startSeq` / `endSeq`。`assistant/message` 没有 `sourceEventSeqs` 字段。
- `tool/ptc-dispatch` 和 `tool/ptc-dispatch-start` 替代 V2 的代码分发事件名。助手消息与尝试保留内嵌 `stream` 数据。

[V3 codec](../../../packages/session/session-format-v2-to-v3/src/codec.ts)负责物理分帧与来源引用解码。[V2 到 V3 规范](../../../packages/session/session-format-v2-to-v3/README.zh.md#v2-to-v3-specification)负责系统消息、局部引用、PTC 名称和规范化 envelope 的转换；其[原生 V3 准入规则](../../../packages/session/session-format-v2-to-v3/README.zh.md#native-v3-admission)定义这些声明之外的验证要求。

<a id="declaration"></a>
## 声明

[Schema 目录](v3.schema.json)。
...
[完整机器目录](v3.schema.json)包含 59 个根类型和 462 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `dt` | 必需 | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
| `index` | 必需 | `number` |
...
| `shadowedSeqs` | 必需 | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
...
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `sourceEventSeqs` | 可选 | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
| `surfaceOp` | 必需 | [`packages/core/session/src/types.ts#SurfaceOp`](#persistence-type-packagescoresessionsrctypestssurfaceop) |
| `time` | 必需 | `number` |
...
|---|---|---|
| `maxTokens` | 必需 | `number` |
| `messageSeqs` | 必需 | [`SessionEventEnvelope[36].sourceEventSeqs`](#persistence-type-sessioneventenvelope36sourceeventseqs) |
...
本目录从已验证的[预发布归档](../releases/README.zh.md)重建所选标签，保留每个根类型及其全部可达类型，且不保留无关类型。仓库的规范化图与摘要辅助函数验证重建结果；没有用当前源码类型替换历史声明。归档的提取与规范化限制仍然适用。

所选官方检查点不代表此后所有 V3 负载修订。此分支已在 V3 的 `approval/decided` 中写入 `allowed-always`；本快照不包含该值。[V4 确认记录](../2026-09-19-desktop-remembered-approval-v4.zh.md)记载了对它的首次正式核对及保留要求。

类型声明不代表 codec 准入或迁移安全性。不透明值、外部插件事件、物理正文编码和运行时关系仍不在本目录的覆盖范围内。

```sh
pnpm run verify-persistence-formats
```

该检查仅使用当前工作树，验证截至当前写入版本的所有格式、schema 完整性、双语声明与生成表。它不重放历史应用。维护规则见[格式索引](README.zh.md)，当前声明见[当前目录](../../persistence-catalog.zh.md)。
