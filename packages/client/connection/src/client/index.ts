...
export function apply(ctx: Context): void {
  ...
  const configured = globals.__DSH_TRANSPORT__
  const fetch = configured?.fetch ?? portalRpcFetch(globals)
  const transport = fetch === undefined ? configured : { ...configured, fetch }
