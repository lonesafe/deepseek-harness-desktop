...
The [PDF.js license check](../../../../packages/client/ui-sidebar-documentpreview/tests/pdf-license-bundle.e2e.ts) inspects the actual npm tarball under the built-artifact smoke gate, which depends on the repository build. Packing cannot run in source-only unit checks: it consumes `lib/client.js` and must verify the published bytes. The built lane fails when that bundle is absent, retains every bundled license assertion, and disables retries for this deterministic artifact check.
