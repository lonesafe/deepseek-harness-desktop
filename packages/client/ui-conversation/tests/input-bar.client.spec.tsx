...
describe('running and lock semantics', () => {
  it('dismisses the primary tooltip when Stop becomes disabled Send', () => {
    const { view, button, session } = bench({ running: true })
    fireEvent.focus(button)
    expect(view.getByRole('tooltip').textContent).toBe('停止生成')
    act(() => { session.set({ ...session.getSnapshot(), running: false }) })
