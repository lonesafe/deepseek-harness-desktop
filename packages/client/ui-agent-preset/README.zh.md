...
第四个表层，独立的设置页（`settings.section`，id 为 `agent-presets`，排在「模型」之后——选模型是日常操作，而组装 agent 是它背后那件塑造部署形态的事）：名单以卡片呈现，复制对话框是创建 preset 的唯一入口，随附组装则在只读查看器中展示。该创作分区仅在回环页面注册；远程 Web 保留普通 preset 选择器和运行中会话标签，但不会显示由回环专属创作 RPC 支撑的控件。
