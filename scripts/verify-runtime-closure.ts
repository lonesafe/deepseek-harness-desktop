 ...
 * peer in its dependency graph. The bundled Python runtime additionally owns
 * the shipped agent presets and must directly supply every active preset plugin.
 * With auto peer installation disabled, either omission can otherwise fail only
 * when Cordis loads the packaged plugin.
 */
...
export async function verifyRuntimeClosure(
  ...
  const verifiesPresets = manifestPath === DEFAULT_RUNTIME_MANIFEST
  const platforms = verifiesPresets
    ? await loadJson<RuntimePlatformManifest>(resolve(root, 'python/sdk-runtime/platforms.json'))
    : {}
  const presetPaths = verifiesPresets ? globSync(AGENT_PRESET_GLOB, { cwd: root }).sort() : []
...
if (import.meta.main) {
  ...
  const manifestPath = values.manifest ?? DEFAULT_RUNTIME_MANIFEST
  ...
  if (result.failures.length > 0) {
    ...
    const presets = manifestPath === DEFAULT_RUNTIME_MANIFEST
      ? `${String(result.presetCount)} agent presets and `
      : ''
    console.log(
      `verify-runtime-closure: ${presets}${String(result.workspacePackageCount)} workspace packages form a closed runtime dependency graph.`,
    )
  }
}
