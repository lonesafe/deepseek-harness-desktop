...
字段是否被覆盖，取决于它是否出现在原始用户层中，而非取决于它的值；重置会清除该字段，使其重新继承组装层。secret 角色的字段绝不搭乘响应，因此密钥控件初始为空、只报告是否已配置，并经由 credentials 领域而非 settings 分节写入；空草稿不写入任何东西，保留已存密钥。远程页面即使遇到 `credentials.describe` 失败也会让凭据控件保持只读，因此传输故障不会意外重新启用桌面隧道明确拦截的操作。
