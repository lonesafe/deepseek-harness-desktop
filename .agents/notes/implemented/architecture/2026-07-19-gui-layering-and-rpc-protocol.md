...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the vite application) + `dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per application (web.ts / headless.ts) + Electron lifecycle and packaging; the vite app is a thin main over the `dsh-client-web` shell surface | Applications use dynamic imports so they never load each other; workspace knowledge like dist and executable location stays in the app |
