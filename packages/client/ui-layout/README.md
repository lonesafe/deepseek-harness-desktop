...
At 640px and below, the sidebar track is removed from the grid so conversation keeps the full viewport width; the mounted sidebar becomes a left overlay, and its collapsed presentation contributes only the floating opener. Explicit grid-column ownership keeps the conversation and details faces in their intended tracks even while the sidebar is absolutely positioned.
