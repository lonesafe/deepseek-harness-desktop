...
The two event WebSockets carry only the host→browser downlink and accept no client application messages. A browser connected directly to the Host continues to use `POST /api/*` for client→host unary calls and `respond`; an authenticated remote-portal shell may select the separately documented [multiplexed `/api/rpc` carrier](2026-08-16-remote-websocket-rpc-carrier.md), which the portal translates into the existing desktop HTTP tunnel request so Host policy remains authoritative. The fixture and `InProcessApiClient(toFetchHandler(api))` continue to implement the same two-stream `IApiClient` abstraction. The in-process fetch carrier retains SSE encoding and decoding to verify the channel-independent protocol's isomorphism, but network GET requests to `/api/events.*` answer only Upgrade Required and do not provide a browser compatibility fallback.
...
A browser abort or socket close cancels the corresponding host stream; plugin teardown also waits for that source iterator's cleanup. If a host stream throws midway, the carrier sends one existing `stream/error` frame and then closes the socket; the client treats that frame as connection loss rather than delivering it to a business sink. Each WebSocket reports open independently, and the existing readiness handshake still waits until mux and host are both open and the `host.describe` unary call has succeeded before publishing connected.

## Verification

Webserver contract tests pin upgrade-pathname dispatch, duplicate-registration rejection, disposal, and teardown; connection real-network tests pin each WebSocket's trust check, open, schema envelope, frame order, stream error, and close cancellation. Client tests prove that downlinks create `ws:`/`wss:` URLs, that direct pages keep unary calls and `respond` on HTTP `fetch`, and that the exact portal marker selects one separate multiplexed socket. The assembled keyless browser replay continues to cover Chromium, a real host, the direct HTTP uplink, and the full WebSocket downlink chain.
...
**Move direct Host unary calls and respond onto the event sockets.** This would couple request timeout, cancellation, HTTP status, trust-fence, and correlation behavior to streams whose only purpose is downlink delivery. The remote portal uses a separate multiplexed socket and retains the logical HTTP-policy boundary through translation; direct Host uplink remains HTTP.
