...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` packages the same Web composition in Electron through a managed random loopback origin; the [desktop launcher note](2026-08-15-electron-desktop-launcher.md) owns that application boundary.

```
apps/*  (applications: web = vite, cli = bin dispatch, desktop = Electron launcher)
  │ consume
  ▼
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser-shaped carriage for ordinary browsers and the Electron renderer; zero workspace dependencies (the registry arrives by structural injection) |
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the Vite application) + `dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per CLI application (web.ts / headless.ts) + Electron lifecycle and packaging; the Vite app and Electron renderer both consume the `dsh-client-web` shell surface | CLI applications use dynamic imports so they never load each other; applications do not alter Host assembly; workspace knowledge such as dist and executable location stays in the app |
...
1. **Pick a fetch impersonation**: browser or desktop-renderer same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (see the "Subclass table" below).
...
The applications preserve the division: the Web command mounts Host, carrier, and browser composition; the desktop launcher owns that command as a child and renders its loopback origin; `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
